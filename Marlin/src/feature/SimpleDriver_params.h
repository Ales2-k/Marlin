...
#define GCONF_INTERNAL_RSENSE          (1 << 1)            //0: Operation with external sense resistors
                                                           ...
                                                           //Reset default: OTP
#define GCONF_EN_SPREAD_CYCLE           (1 << 2)            //0: stealthChop PWM mode enabled (depending on velocity thresholds). Initially switch from off to on state while in stand still, only.
                                                           //1: spreadCycle mode enabled
                                                           //A high level on the pin SPREAD_ (TMC222x, only) inverts this flag to switch between both chopper modes.
                                                           //Reset default: OTP
...
#define OTP_PROG_OTPMAGIC              (255 << 8)          //Set to 0xbd to enable programming. A programming time of minimum 10ms per bit is recommended (check by reading OTP_READ_).
#define OTP_PROG_OTPMAGIC_SHIFT        8                   //

//Register                             Address   Type      N Bits    Description
/********************************************************************************/
#define OTP_READ__REGISTER              0x05      //R       24        //Access to OTP memory result and update
                                                                     //7..0: OTP0 byte 0 read data
                                                                     //15..8: OTP1 byte 1 read data
                                                                     //23..16: OTP2 byte 2 read data
/********************************************************************************/
//Function                             Bit                 Description
#define OTP_READ__EN_SPREAD_CYCLE        (1 << 23)           //otp2.7: This flag determines if the driver defaults to spreadCycle or to stealthChop.
...
#define OTP_READ__IHOLD                 (3 << 21)           //otp2.6, otp2.5: Reset default for standstill current IHOLD (used only if current reduction enabled, e.g. pin PDN_UART low).
#define OTP_READ__IHOLD_SHIFT           21                  //%00: IHOLD= 16 (53% of IRUN)
                                                           //%01: IHOLD= 2  ( 9% of IRUN)
                                                           //%10: IHOLD= 8  (28% of IRUN)
                                                           //%11: IHOLD= 24 (78% of IRUN)
                                                           //Reset default for run current IRUN=31
#define OTP_READ__IHOLDDELAY            (3 << 19)           //otp2.4, otp2.3: Reset default for IHOLDDELAY
#define OTP_READ__IHOLDDELAY_SHIFT      19                  //%00: IHOLDDELAY= 1
                                                           //%01: IHOLDDELAY= 2
                                                           //%10: IHOLDDELAY= 4
                                                           //%11: IHOLDDELAY= 8
#define OTP_READ__PWM_FREQ              (1 << 18)           //otp2.2: Reset default for PWM_FREQ:
                                                           //0: PWM_FREQ=%01=2/683
                                                           //1: PWM_FREQ=%10=2/512
#define OTP_READ__PWM_REG               (1 << 17)           //otp2.1: Reset default for PWM_REG:
                                                           //0: PWM_REG=%1000: max. 4 increments / cycle
                                                           //1: PWM_REG=%0010: max. 1 increment / cycle
#define OTP_READ__PWM_OFS               (1 << 16)           //otp2.0: Depending on otp_en_spreadCycle:
...
#define OTP_READ__TPWMTHRS              (7 << 13)           //otp1.7, otp1.6, otp1.5: Depending on otp_en_spreadCycle:
#define OTP_READ__TPWMTHRS_SHIFT        13                  //0: Reset default for TPWM_THRS as defined by (0..7):
...
#define OTP_READ__PWM_AUTOGRAD          (1 << 12)           //otp1.4: Depending on otp_en_spreadCycle;
...
#define OTP_READ__PWM_GRAD              (15 << 8)           //otp1.3, otp1.2, otp1.1, otp1.0: Depending on otp_en_spreadCycle:
#define OTP_READ__PWM_GRAD_SHIFT        8                   //0: Reset default for PWM_GRAD as defined by (0..15):
...
#define OTP_READ__TBL                   (1 << 7)            //otp0.7: Reset default for TBL:
                                                           //0: TBL=%10
                                                           //1: TBL=%01
#define OTP_READ__INTERNALRSENSE        (1 << 6)            //otp0.6: Reset default for GCONF.internal_Rsense
                                                           //0: External sense resistors
                                                           //1: Internal sense resistors
#define OTP_READ__OTTRIM                (1 << 5)            //otp0.5: Reset default for OTTRIM:
                                                           //0: OTTRIM= %00 (143°C)
                                                           //1: OTTRIM= %01 (150°C)
                                                           //internal power stage temperature about 10°C above the sensor temperature limit
#define OTP_READ__FCLKTRIM              (31 << 0)           //otp0.4, otp0.3, otp0.2, otp0.1, otp0.0: Reset default for FCLKTRIM
#define OTP_READ__FCLKTRIM_SHIFT        0                   //0: lowest frequency setting
...
#define IOIN_SPREAD__TMC222X            (1 << 2)            //1: SPREAD_ pin high state for TMC222X driver
...
//Register                             Address   Type      N Bits    Description
/********************************************************************************/
#define VACTUAL_REGISTER               0x22      //W       24        //VACTUAL allows moving the motor by UART control. It gives the motor velocity in +-(2^23)-1 [steps / t]
